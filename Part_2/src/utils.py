...
def game1_running(clock, success, game_text, player1, player2, results):
    if 0 <= clock <= 20:
        success = True
...
def get_current_option_and_warming(frame, text_frame):
    ...
    if len(results.multi_hand_landmarks) == 1:
        ...
        if landmark_condition.is_five(hand_landmarks):
            return 5
        if landmark_condition.is_one(hand_landmarks):
            return 1
        if landmark_condition.is_three(hand_landmarks):
            return 3
        if landmark_condition.is_two(hand_landmarks):
            return 2
        else:
            return None
...
def detect_rock_gestures(hand_landmarks):
